...
export default function PhotosModal({ project, onClose }) {
  ...
  const [lightbox,  setLightbox]  = useState(null) // url for full-screen view
      ...
      <div className="fixed inset-0 z-50 flex flex-col bg-black/60 backdrop-blur-sm">
        ...
        <div className="bg-white rounded-t-2xl max-h-[90vh] flex flex-col shadow-2xl">
          ...
          <div className="flex items-center justify-between px-4 pb-3 pt-1 border-b border-slate-100 flex-shrink-0">
            <div>
              <h2 className="text-lg font-bold text-slate-800">📷 Снимки</h2>
          ...
          {!loading && (
            <div className="px-4 py-2 border-b border-slate-100 flex-shrink-0">
              <p className="text-xs text-slate-400">
                {photos.length === 0 ? 'Няма снимки' : `${photos.length} снимки`}
              </p>
            </div>
          )}
          ...
          <div className="flex-1 overflow-y-auto thin-scroll p-4">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-14 text-center">
                ...
                <p className="text-slate-400 text-sm mb-1">Няма добавени снимки</p>
                <p className="text-slate-300 text-xs">Натисни „+ Добави снимки" по-долу</p>
              </div>
            ) : (
...
function PhotoCard({ photo, onView, onDelete, onCaptionChange }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-slate-100 shadow-sm bg-white">
      {/* Image */}
      <div
        className="aspect-square bg-slate-100 cursor-pointer overflow-hidden relative group"
        onClick={onView}
      >
        <img
          src={photo.url}
        ...
        {editCap ? (
          ...
          <div className="flex items-center justify-between gap-1">
            <button
              ...
              className="text-xs text-slate-400 hover:text-slate-700 truncate text-left flex-1"
            >
              {photo.caption || <span className="italic">Добави надпис...</span>}
            </button>
            <button
              onClick={onDelete}
