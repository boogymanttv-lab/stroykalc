...
export default function TasksModal({ project, onClose }) {
  ...
  async function loadTasks() {
    ...
    if (navigator.onLine) {
      ...
      if (data) { await db.tasks.bulkPut(data); setTasks(data) }
    } else {
  ...
  const STATUS = {
    todo:        { icon: '⬜', next: 'in_progress' },
    in_progress: { icon: '🔄', next: 'done'        },
    done:        { icon: '✅', next: 'todo'         },
  }
  ...
  async function addTask() {
    ...
    const record = await offlineInsert('tasks', {
      ...
      title, due_date: newDate || null,
      status: 'todo', created_at: new Date().toISOString(),
    })
    setTasks(t => [...t, record])
  ...
  async function updateTitle(id, title) {
    ...
    setTasks(ts => ts.map(x => x.id === id ? { ...x, title } : x))
  }

  const todo    = tasks.filter(tk => tk.status !== 'done')
  const done    = tasks.filter(tk => tk.status === 'done')
  const overdue = todo.filter(tk => tk.due_date && new Date(tk.due_date) < new Date())
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-black/60 backdrop-blur-sm">
      ...
      <div className="bg-white rounded-t-2xl max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between px-4 pb-3 pt-1 border-b border-slate-100 flex-shrink-0">
          <div>
            <h2 className="text-lg font-bold text-slate-800">{t('tasksTitle')}</h2>
            <p className="text-xs text-slate-400 truncate max-w-[220px]">{project.name}</p>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400">
              {todo.length} {lang === 'en' ? 'active' : 'активни'} · {done.length} {lang === 'en' ? 'done' : 'готови'}
            </span>
        ...
        {overdue.length > 0 && (
          <div className="mx-4 mt-3 flex-shrink-0 bg-red-50 border border-red-200 rounded-xl px-4 py-2.5 text-sm text-red-700 font-semibold">
            ⚠️ {overdue.length} {lang === 'en' ? `task${overdue.length > 1 ? 's' : ''} overdue!` : `задача${overdue.length > 1 ? 'и' : ''} с изтекъл срок!`}
          </div>
        )}
...
function TaskRow({ task, STATUS, lang, onToggle, onDelete, onTitleChange }) {
  ...
  return (
    ...
    >
      <button onClick={onToggle} className="text-xl leading-none flex-shrink-0 mt-0.5 transition-transform hover:scale-110">
        {st.icon}
      </button>
      <div className="flex-1 min-w-0">
        ...
        {task.due_date && (
          <div className={`text-xs mt-0.5 font-semibold ${isOverdue ? 'text-red-500' : 'text-slate-400'}`}>
            ...
            {isOverdue && (lang === 'en' ? ' — overdue!' : ' — просрочена!')}
          </div>
        )}
      </div>
      <button onClick={onDelete} className="text-red-300 hover:text-red-500 text-xl leading-none flex-shrink-0">×</button>
    </div>
  )
}
